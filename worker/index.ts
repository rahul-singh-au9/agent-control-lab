  ...
  const tooLarge = () =>
    new HttpError(
      ...
      limit === 1_024
        ...
        : 'Trace exceeds the 64 KiB limit.',
    );
  ...
  while (true) {
    ...
    if (size > limit) {
      try {
        ...
        /* Rejection must not turn an oversized input into a storage error. */
      }
      throw tooLarge();
    }
    chunks.push(value);
...
async function api(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  const allowedMethods =
    path === '/api/health'
      ? ['GET']
      : path === '/api/session' || path === '/api/reports'
        ? ['GET', 'POST']
        : uuid.test(id)
          ? ['GET', 'DELETE']
          : undefined;
  if (!allowedMethods) throw new HttpError(404, 'Report or endpoint not found.');
  ...
  if (path === '/api/reports' && method === 'GET') {
    const rows = await env.DB.prepare(
      'SELECT id, title, origin, created_at, action_count FROM reports WHERE owner_id = ? AND expires_at > ? ORDER BY created_at DESC, id DESC LIMIT 20',
    )
      .bind(owner, now)
      .all<ReportRow>();
  ...
  if (path === '/api/reports' && method === 'POST') {
    ...
    const result = await env.DB.prepare(
      `INSERT INTO reports (id, owner_id, title, origin, created_at, expires_at, action_count, trace)
      SELECT ?, ?, ?, ?, ?, ?, ?, ?
    ...
    if (!result.meta.changes)
      throw new HttpError(
        409,
        'Storage limit reached. Export and delete older reports, or keep auditing locally.',
      );
    return json(
      {
        report: {
  ...
  if (method === 'GET') {
    const row = await env.DB.prepare(
      'SELECT id, title, origin, created_at, action_count, trace FROM reports WHERE id = ? AND owner_id = ? AND expires_at > ?',
    )
      .bind(id, owner, now)
      .first<ReportRow>();
    if (!row) throw new HttpError(404, 'Report not found in this browser workspace.');
    return json({ report: { ...metadata(row), trace: parseTrace(JSON.parse(row.trace!)) } });
  }
  if (method === 'DELETE') {
    const result = await env.DB.prepare(
      'DELETE FROM reports WHERE id = ? AND owner_id = ? AND expires_at > ?',
    )
      .bind(id, owner, now)
      .run();
    if (!result.meta.changes)
      throw new HttpError(404, 'Report not found in this browser workspace.');
    return json({ ok: true });
  }
...
export default {
      ...
      return json(
        {
          error:
            error instanceof HttpError
              ? error.message
              : 'Saved reports are temporarily unavailable. You can still audit and export locally.',
          requestId,
        },
        status,
        {
  ...
  async scheduled(_event: ScheduledController, env: Env): Promise<void> {
    const result = await env.DB.prepare('DELETE FROM reports WHERE expires_at <= ?')
      .bind(Date.now())
      .run();
    // D1 includes counter-trigger updates in changes, not just deleted reports.
    console.log(
      JSON.stringify({
