...
function unsupported(action: ProposalEvent): Decision | null {
  return action.tool === SUPPORTED_TOOL
    ? null
    : decision(
        action,
        'unsupported',
        'unsupported-tool',
        `The tool “${action.tool}” has no evaluator. No safety conclusion is available.`,
      );
}
...
function missingGrant(action: ProposalEvent, coverage: Trace['coverage']): Decision {
  return coverage.authorization === 'complete'
    ? decision(
        ...
        'No authoritative grant matches the claimed grant ID in the recorded authorization history.',
      )
    : decision(
        action,
        'review',
        'incomplete-authorization',
        'No matching grant is recorded, and authorization coverage is partial.',
      );
}
...
function scopeFailure(action: ProposalEvent, grant: GrantEvent): Decision | null {
  if (action.actor !== grant.actor || action.session !== grant.session) {
    return decision(
      ...
      'block',
      'principal-mismatch',
      'This grant belongs to a different actor or session.',
      [grant.id],
    );
  }
  if (action.resource !== grant.resource || action.destination !== grant.destination) {
    return decision(
      ...
      'block',
      'scope-mismatch',
      'The proposed artifact or destination is outside the grant scope.',
      [grant.id],
    );
  }
  return null;
...
function evaluateStatic(
  ...
  if (!grant) {
    return decision(
      action,
      coverage.authorization === 'complete' ? 'block' : 'review',
      'missing-static-grant',
      'No matching grant exists in the baseline’s frozen snapshot from before the first proposal. Grants recorded later are ignored.',
    );
  }
  ...
  return decision(
    ...
    'allow',
    'static-scope-match',
    'Matches a grant recorded before the first proposal. This baseline ignores later grants, revocation, expiry, usage and content state.',
    [grant.id],
  );
}
...
function evaluateCurrent(
  action: ProposalEvent,
  grants: Map<string, LedgerGrant>,
  resources: Map<string, StateEvent>,
  coverage: Trace['coverage'],
): Decision {
  const notSupported = unsupported(action);
  if (notSupported) return notSupported;
  ...
  if (entry.revokedBy)
    return decision(
      ...
      'block',
      'revoked-grant',
      'The grant was revoked before this proposal.',
      [grant.id, entry.revokedBy],
    );
  if (Date.parse(action.timestamp) >= Date.parse(grant.expiresAt)) {
    return decision(
      ...
      'block',
      'expired-grant',
      'The grant had expired at the recorded proposal time.',
      [grant.id],
    );
  }
  if (entry.dispatchIds.length >= grant.maxUses) {
    return decision(
      ...
      'block',
      'exhausted-grant',
      'The grant’s dispatch allowance was already consumed. Failed dispatched attempts also consume a use.',
      [grant.id, ...entry.dispatchIds],
    );
  }
  if (action.version !== grant.version || action.digest !== grant.digest) {
    return decision(
      ...
      'block',
      'unapproved-content',
      'The proposed content version or digest differs from the approved content.',
      [grant.id],
    );
  }
  ...
  if (!state) {
    return coverage.resourceState === 'complete'
      ? decision(
          ...
          'block',
          'missing-resource',
          'No state for this artifact exists in the complete recorded resource history.',
          [grant.id],
        )
      : decision(
          action,
          'review',
          'incomplete-resource-state',
          'The artifact’s state is not recorded and resource coverage is partial.',
          [grant.id],
        );
  }
  if (state.version !== action.version || state.digest !== action.digest) {
    return decision(
      ...
      'The proposed publication is bound to an older or different artifact state.',
      [grant.id, state.id],
    );
  }
  if (coverage.authorization === 'partial' || coverage.resourceState === 'partial') {
    return decision(
      ...
      'review',
      'incomplete-coverage',
      'Recorded fields match, but partial coverage cannot establish that intervening revocations or state changes are absent.',
      [grant.id, state.id],
    );
  }
  return decision(
    ...
    'allow',
    'current-state-match',
    'Matches the current recorded artifact and a valid, unconsumed grant for this actor, session and destination.',
    [grant.id, state.id],
  );
}
...
function measure(decisions: Decision[], labels: Trace['labels']): PolicyMetrics {
  ...
  const count = (items: Decision[], value: DecisionValue) =>
    items.filter((item) => item.decision === value).length;
  ...
  return {
    ...
    permittedInterrupted: ratio(
      count(permitted, 'block') + count(permitted, 'review'),
      permitted.length,
    ),
...
export function evaluateTrace(input: Trace): Evaluation {
  ...
  for (const event of trace.events) {
    if (event.type === 'grant') {
      ...
      if (
        grant &&
        action.tool === SUPPORTED_TOOL &&
        !scopeFailure(action, grant.event) &&
        action.version === grant.event.version &&
        action.digest === grant.event.digest
      ) {
  ...
  if (trace.origin === 'fixture')
    warnings.unshift(
      'This is an authored example fixture, not a captured agent run or research result.',
    );
  if (trace.coverage.authorization === 'partial' || trace.coverage.resourceState === 'partial') {
    warnings.push(
      'Coverage is partial. Matching known fields cannot establish full authorization or current state.',
    );
  }
    ...
    policies: [
      {
        ...
        metrics: measure(staticDecisions, trace.labels),
      },
      {
        id: 'current-state',
        name: 'Current state',
        decisions: currentDecisions,
        metrics: measure(currentDecisions, trace.labels),
      },
    ],
    warnings,
  };
