...
await writeFile(
  `${output}/index.html`,
  `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Agent Control Lab — local walkthrough</title><style>body{font:16px/1.6 system-ui,sans-serif;background:#f5f7f2;color:#182822;max-width:1100px;margin:40px auto;padding:0 24px}h1{font-size:32px;line-height:1.2}h2{font-size:23px}section{background:white;padding:24px;border:1px solid #d8ded5;border-radius:16px;margin:24px 0}video{display:block;width:100%;max-height:740px;background:#111827;border-radius:8px}button{font:inherit;text-align:left;color:#165649;background:none;border:0;padding:5px;cursor:pointer}button:hover{text-decoration:underline}button:focus-visible{outline:2px solid #165649}button span{font-variant-numeric:tabular-nums;display:inline-block;min-width:55px}a{color:#165649}li p{color:#46564e;margin:0 0 8px 60px}ol{list-style:none;padding:0}small{color:#46564e}</style><h1>Agent Control Lab</h1><p>Actual local browser recordings, captured on ${escapeHtml(manifest.recordedAt.slice(0, 10))}. Save, reload, open and delete use the real local Worker API and D1 database. All examples are synthetic.</p><p>The storage-failure segment deliberately injects HTTP 503 responses. Captions occupy an added strip below the original browser viewport. The workflows are continuous, without sped-up or reconstructed interactions; there is no audio track.</p>${panels}<small>Local verification only. These recordings do not show a public deployment or live agent enforcement.</small><script>document.querySelectorAll('button[data-video]').forEach(button=>button.addEventListener('click',()=>{const video=document.getElementById(button.dataset.video);video.currentTime=Number(button.dataset.seconds);video.play().catch(()=>{});video.scrollIntoView({behavior:'smooth',block:'center'});}));</script></html>`,
);
